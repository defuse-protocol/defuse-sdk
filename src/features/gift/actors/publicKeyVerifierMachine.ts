    ...
    adding: {
      invoke: {
        ...
        input: ({ context, event }) => {
          ...
          return {
            accountId: context.nearAccount.accountId,
...
async function checkPublicKeyOnchain({
  ...
  // Check if the public key exists in the keys array
  return response.keys.some(
    (key: { public_key: string }) => key.public_key === nearAccount.publicKey
  )
}
